...
class PortalRelocator {
  ...
  Future<RelocationResult> relocate({
    ...
    final future = _run(current, identity, suggestedPort).then((result) {
      _last = result;
      return result;
    }).whenComplete(() {
      _running = null;
  ...
  Future<RelocationResult> _run(
      String current, PortalIdentity identity, int? suggestedPort) async {
    if (!identity.isKnown) {
    ...
    if (search != null && identity.id.isNotEmpty) {
      final servers = await search(identity.id).catchError(
  ...
  static Future<List<DiscoveredServer>> searchLan(String portalId) async {
    final token = DiscoveryCancelToken();
    final result = await const ServerDiscovery().discover(
      cancelToken: token,
      onUpdate: (servers) {
        if (servers.any((s) => s.id == portalId)) token.cancel();
      },
    );
